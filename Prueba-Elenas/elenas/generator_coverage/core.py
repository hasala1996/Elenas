...
def start_swith(name, tests, clv):
    if name.startswith(clv):
        tests.append(name)
    return tests
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for base in bases:
            ...
            for member_name, member in inspect.getmembers(base):
                ...
                tests = start_swith(member_name, tests, '_test_')
                # get the users
        ...
        for attr in attrs:
            # get the functions starting with _test_
            tests = start_swith(attr, tests, '_test_')
        # get the users from this class
        if 'users' in attrs:
            users += attrs['users']
        for test in tests:
            for username, password in users:
                attrs[cls.get_function_name(test, username)] = cls.generate_test(test, username, password)
